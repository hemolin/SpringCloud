...
public class TicketServiceImpl implements TicketService {
     ...
     */
    @HystrixCommand(
            fallbackMethod = "queryGoodsFail",
            commandProperties = {
                @HystrixProperty(name = "execution.isolation.strategy", value = "THREAD"),
            },
            threadPoolKey = "queryGoods",
            threadPoolProperties = {
                    @HystrixProperty(name = "coreSize", value = "100")
            })
    @Override
    public String queryGoods() {
        String url = String.format("http://%s/%s", "micro-goods", "queryGoods");
        String result = restTemplate.postForObject(url, null, String.class);
        return result;
    }

    public String queryGoodsFail(){
//        atomicInteger.getAndAdd(1);
        log.info("queryGoodsFailxxxxxxxxxxx"+atomicInteger.get());
        return "queryGoodsFailxxxxxxxxxxx";
    }
}
